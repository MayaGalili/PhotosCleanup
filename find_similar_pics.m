...
for id = 1:height(files)
    ...
    if prev_sz == curr_sz
        [pics_to_delete_sz] = compar_photos(dir_nn, curr_nm, prev_nm, pics_to_delete_sz);
    end
...
if pics_to_delete_sz > 0
    fprintf('You can delete %d files [now starting with ''_'']! \n', pics_to_delete_sz);
else
    fprintf('I can not find files to delete. \n');
end
end
...
function [pics_to_delete_sz] = compar_photos(dir_nn, curr_nm, prev_nm, pics_to_delete_sz)
...
if (isequal(Image1,Image2))
    [pics_to_delete_sz] = rename_duplicated_photo(prev_file_path, dir_nn, prev_nm, curr_nm, pics_to_delete_sz);
else
...
function [pics_to_delete_sz] = rename_duplicated_photo(prev_file_path, dir_nn, prev_nm, curr_nm, pics_to_delete_sz)
